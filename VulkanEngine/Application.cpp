...
bool Application::isDeviceSutable(VkPhysicalDevice device)
{
...
void Application::createSurface()
{
	if (glfwCreateWindowSurface(instance, window, nullptr, &surface) != VK_SUCCESS) {
		throw std::runtime_error("KHR window surface creation failed!");
	}
}
...
void Application::createSwapChain()
{
	...
	VkPresentModeKHR presentMode = chooseSwapChainPresentMode(swapChainSupport.presentMode);
	VkExtent2D extent = chooseSwapExtent(swapChainSupport.capablities);
	uint32_t imageCount = swapChainSupport.capablities.minImageCount + 1;

	if (swapChainSupport.capablities.maxImageCount > 0 && imageCount >
		swapChainSupport.capablities.maxImageCount) {
	...
	if (vkCreateSwapchainKHR(device, &createInfo, nullptr, &swapChain) != VK_SUCCESS) {
		throw std::runtime_error("failure in trying to create the swap chain");
	}
...
void Application::createGraphicsPipeline()
{
	...
	/* IF SOMETHING DOES'NT WORK THE WAY IT SHOULD COME BACK HERE */
...
std::vector<char> Application::readFile(const std::string& filename)
{
	...
	if (!file.is_open()) {
		throw std::runtime_error("Error no such file or directory '" + filename + "'!");
	}
...
void Application::recordCommnadBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
{
...
void Application::drawFrame()
{
	...
	recordCommnadBuffer(commandBuffers[currentFrame], imageIndex);
	...
	if (vkQueueSubmit(graphicsQueue, 1, &submitInfo, inFlightFences[currentFrame]) != VK_SUCCESS) {
		throw std::runtime_error("failed to submit draw commnad buffer");
	}
